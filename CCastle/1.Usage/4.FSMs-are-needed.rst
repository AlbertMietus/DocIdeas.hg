...
A curious transition is the “`instantly` :math:`\epsilon`\-transition”; its needs no input -- and so (when used) the FSM_
becomes a NFA_. But when this is the only ``rule`` for a ``state``; it does not make a FSM_ non-deterministic and is easy
to implement in software: :code:`when state==C: { state:=N; act_appropriate(); }`. This is often easier then removing
that state and combining the actions (one step back in all paths).
...
This does nog make the FSM_ easier to design nor maintain! Wouldn't it be great to a kind generic “on-state” with one
...
So the :const:`on` state is kind of split in to :const:`booting`, :const:`warming-up`  and :const:`operational`. And
...
number -- but the are independent and concurent.
...
state). And we can even use ‘Entry’ and ‘Leave’ (or *exit* -- but that has the same 1-character abbreviation).
