...
So, to make a FSM-code compact, we have to focus on the rules.
|BR|
It’s tempting to regard this rule-set as a *matrix*: with (‘:math:`S`’) and (‘:math:`E`’) on the axes and the
next-states [#non-determinism]_ (and actions) inside each cell. Nevertheless this matrix is huge and probably sparse:
most cells are empty, as the combination does not occur.

We can describe such a sparse-matrix by listing the cell-value with the coordinates. This boils-down to listing the
rules, one by one.
|BR|
The disadvantage of this that many states and/or events are listed many times. As we will see below, some languages
solve this by factor-out `states` and/or `events`.
