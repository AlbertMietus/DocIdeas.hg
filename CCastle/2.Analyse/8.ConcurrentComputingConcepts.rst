...
<https://en.wikipedia.org/wiki/Broadcasting_(networking)>`_ (and al variants that are not point-to-point) focus on
reducing the amount of data on the network itself. In distributed computing `Broadcasting
<https://en.wikipedia.org/wiki/Broadcast_(parallel_pattern)>`_ is a parallel Design pattern. Whereas the `Broadcast flag
<https://en.wikipedia.org/wiki/Broadcast_flag>`_  in TV steaming is a complete other idea: is it allowed to save
