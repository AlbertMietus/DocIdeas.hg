...
|BR| In :ref:`MPA-examples` some existing messaging passing systems are classified in those therms, for those that do
prefer a more practical characterisation.
...
Despide it’s is not truly a characteristic of the messages itself, messages can be *buffered*, or not. It is about
piping, transporting the message: can this “connection” (see below) *contain/save/store* messages?  When there is no
storage at all the writer and reader needs to rendezvous: send and receive at the same (wall) time.
|BR|
...
Messages can be send over (pre-) *connected channels* or to freely addressable end-points. Some people use the term “connection
oriented” for those connected-channels, others use the term “channel” more generic and for any medium that is
...
itself.
|BR|
A big advantage is, that one does not need to create channels and end-points first -- which especially count when a low
number (possible one) of messages are send to the same receiver, and/or many receivers exist (which would lead to a huge
number of channels).
...
Messages --or actually the channel [#channelDir]_ that transport them-- can be *unidirectional*: from sender to receiver only;
...
.. hint::
   ...
   To make that connection reliable, we need checksums, low-level conformation message, and once in a while have to sent
   a message again. This implies some buffering (at both sides), and so the audio-stream will have a bit of delay.
   This is a  common solution for unidirectional POD-casts, and such.

   For a bidirectional conversation however this buffering is not satisfactory. It makes the *slow*, people have to wait
   on each-other and will interrupted one-other.
   |BR|
   Then, a *faster* conversation with a bit of noise is commonly preferred.a
...
.. [#channelDir]
   Strictly speaking, one should not use  “channel” here. Non the less, the “direction of a medium”, that is not a
   (connected) channel sounds void. Although we do not exclude it, we use a pragmatic way of writing.
