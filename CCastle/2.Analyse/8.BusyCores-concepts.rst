...
.. post::
   ...
   :need:`U_ManyCore`].
   |BR|
   We also discused threads_, they do not scale well for CPU-bound (embedded) systems, and that er are contemporary,
   abstractions on top of them --although the not always fit nicely in existing languages.
