...
Triggering Events (to sub-elements
----------------------------------
.. code-block:: jinja

   {%macro send_event(outPort, event, ofProtocol, withParms) -%}
...
Here, we assume ‘withParms’ is already correctly formated: a comma (‘,’) separated list of values; e.g in python::
  ``", ".join(for p in parms)``
